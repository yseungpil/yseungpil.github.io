...
- 플레이어는 원거리 투사체를 발사하여 공격하며 오른쪽과 위로만 공격 가능해 위치를 신중하게 잡도록 한다.
- 적은 여러 패턴의 탄막을 발사하며 맵 곳곳을 이동한다.
- 탄막 이외의 브레스, 칼날, 등 공격패턴 추가
- 2	페이즈
- 1페이즈와 2페이즈로 나누어 난이도를 조절하고 상태와 패턴에 변화를 준다.

3	플랫폼 변화
- 플랫폼을 이동시키거나 소멸, 생성을 통해 플랫폼이 가만히 있는 것이 아닌 플레이어가 상호작용 할 수 있도록 변경
4 이동 패턴 추가
- 적이 정해진대로 움직이는 것이 아닌 플레이어의 위치에 따라 이동하는 형태 추가
- 플레이어를 쫓아 플랫폼을 오르기도 하는 졸개 추가
